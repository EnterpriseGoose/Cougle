...
import { IGameInterface, IKeyBoardEvent } from "./game-interface";

var words = gameWords;

async function getLatestPlay(props: IGameInterface, date = new Date()) {
  const { db, uid } = props;
  if (!uid) return;
...
const useGameHook = (props: IGameInterface) => {
  ...
  const initialKeyStates = Array.from(Array(SETTING.COUNT_OF_TRY), () =>
    new Array(SETTING.LENGTH_OF_WORD).fill(KEY_STATE.EMPTY)
  );
  const [keyStates, setKeyStates]: any = useState(initialKeyStates);
  ...
  useEffect(() => {
    if (cookies.guessedWords && cookies.keyStates && cookies.selectedLetters) {
      setGuessedWords(cookies.guessedWords);
      setKeyStates(cookies.keyStates);
  ...
  const pressLetter = (event: IKeyBoardEvent | string) => {
    if (number < SETTING.LENGTH_OF_WORD && !gameFinished) {
      ...
      keyStates[tryCount][number] = KEY_STATE.TBD;
    ...
    if (!gameFinished) {
      if (number < SETTING.LENGTH_OF_WORD) {
        ...
        if (words.includes(guessedWords[tryCount].join(""))) {
          ...
          setCookie("keyStates", JSON.stringify(keyStates));
  ...
  const pressBackspace = () => {
    if (number > 0 && !gameFinished) {
      keyStates[tryCount][number - 1] = KEY_STATE.EMPTY;
